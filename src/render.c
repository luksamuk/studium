...
void
st_texture_render(st_texture* img, st_vec2 pos)
{
...
st_gpu_shader
st_shader_from_file(st_shader_type type, const char* filename)
{
    if(!filename) {
        st_log_err("NULL reference to file name");
        return 0;
    }
    ...
    fp = fopen(filename, "r");
    if(!fp) {
        st_log_err_f("Cannot open shader file: %s\n", strerror(errno));
        return 0;
    }
    ...
    char* source = malloc(sz * sizeof(char));
    fread(source, sizeof(char), sz, fp);
    fclose(fp);

    st_gpu_shader compiled_shader = st_shader_compile(type, source);
    free(source);
